...
def test_sort_by_price(page):
    steps = CatalogSteps(page)
    steps.login("standard_user", "secret_sauce").sort_items('lohi')
...
def test_add_to_cart(page):
    steps = CatalogSteps(page)
    steps.login("standard_user", "secret_sauce").add_to_cart("Sauce Labs Bike Light")
    assert steps.get_cart_count() == 1


def test_add_to_cart_sauce_labs_onesie(page):
    steps = CatalogSteps(page)
    steps.login("standard_user", "secret_sauce").add_to_cart("Sauce Labs Onesie")
    assert steps.get_cart_count() == 1
...
def test_product_details_onesie(page):
    ...
    # Jacket (перенос из другого теста для сокращения кол-ва тестов)
...
def test_remove_item_from_cart(page):
    steps = CatalogSteps(page)
    steps.login("standard_user", "secret_sauce").add_to_cart("Test.allTheThings() T-Shirt (Red)")
